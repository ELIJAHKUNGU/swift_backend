...
exports.doCreateUser = async (firstName, secondName, userEmail, userPhone, password, type, req, res) => {
    bcrypt.hash(password, 0, (err, pinHashed) => {
        ...
        const user = new userModel({ firstName: firstName, secondName: secondName, userEmail: userEmail, password: pinHashed, lastName: lastName, userName: userName, role: "Customer", userPhone: userPhone, address: "0000", city: "Lusaka", state: "Lusaka", zipCode: "0000", country: "Zambia", defaultCurrency: "ZMW", defaultBusiness: "0000" });
        user.save()
            .then(result => {
                ...
                if(type === "merchant"){
                    ...
                    let customerData = {
                        customerName: result.firstName + " " + result.secondName,
